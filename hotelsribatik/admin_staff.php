<?php
    ...
    if (isset($_POST['search'])) {
        $query = $_POST['query'];
        $searchSQL = "SELECT * FROM STAFF WHERE CONCAT(`STAFFNAME`, `FULLNAME`, `EMAIL`, `STATUS`) LIKE '%" . $query . "%'";
...
<html>
    <head>
        <title>Staff of Hotel Sri Batik</title>
    ...
    <body>
        ...
        <main>
            ...
            <div class="container">
                ...
                <div class="btn">
                    <button onclick="document.location='admin_staffregister.php'" class="register_btn">Register Staff</button>
                </div><br>
                <div class="searchbox">
                ...
                <div class="table">
                    <table class="stafftable">
                        ...
                        <tr>
                            ...
                            <?php } ?>
                            <td>
                                <button class="action__btn edit" onclick="document.location='admin_staffedit.php?staffName=<?php echo $row['STAFFNAME']; ?>'">Edit</button>
                                <?php
                                if ($row["STATUS"] == "ONLINE") {
                                    $functionClick = "deleteError()";
                                    echo "<button class='action__btn delete' onclick=\"$functionClick\">Delete</button>";
                                } else {
                                    $functionClick = "confirmDelete('{$row['STAFFID']}', '{$row['STAFFNAME']}')";
                                    echo "<button class='action__btn delete' onclick=\"$functionClick\">Delete</button>";
                                }
                                ?>
                            </td>
        ...
        <script>
            function confirmDelete(staffId, staffName) {
                var confirmation = confirm("Are you sure you want to delete " + staffName + "?");

                if (confirmation) {
                    var url = "delete_function.php?staffId=" + staffId + "&staffName=" + staffName + "&editType=staff";
                    document.location = url;
                }
            }
            function deleteError() {
                alert("Sorry, the account you try to delete is ONLINE.\n");
            }
        </script>
    </body>
</html>