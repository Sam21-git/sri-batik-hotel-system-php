<?php
    ...
    //check if there is no staffname in session, cant enter this page
    if(!$adminName){
        echo "<script>alert('Please login first!'); window.location.href = 'admin_login.php';</script>";
    }

    // get sale data from db to count total sale for each month
    $saleSQL = "SELECT * FROM PAYMENT";
    $saleResult = mysqli_query($connect, $saleSQL);

    //check if the query was successful
    $salesByMonth = array();
    while ($row = mysqli_fetch_array($saleResult)) {
        $month = date("F", strtotime($row["PAYDATE"]));
        $paymentAmount = $row['PAYTOTAL'];

        if (!isset($salesByMonth[$month])) {
            $salesByMonth[$month] = 0;
        }

        $salesByMonth[$month] += $paymentAmount;
    ...
    // get room datet to count how many occupied and available
    $roomSQL = "SELECT ISAVAILABLE FROM ROOM";
...
<html>
    <head>
        ...
        // draw sale chart when loaded
        google.charts.setOnLoadCallback(saleChart);

        // draw room chart when loaded
        google.charts.setOnLoadCallback(roomChart);

        // callback that draws the room chart
        function roomChart() {
            var data = google.visualization.arrayToDataTable([
            ['Room', 'Number of room'],
            <?php
                echo "['Available', " . $numAvailable . "],";
                echo "['Occupied', " . $numOccupied . "]";
            ?>
            ]);
        ...
        function saleChart() {
            ...
            ['Month', 'Sales'],
            <?php
            foreach($salesByMonth as $monthName => $totalSale) {
                echo "['" . $monthName . "', " . $totalSale . "],";
            }
            ?>
            ]);
    ...
    <body>
        ...
        <main>
            <h1>Welcome <?php echo "Admin " . $adminName ?>.</h1>

            <div class="container">
            <div class="checkout__soon" id="checkoutSoon">
                <div class="title">
                    Upcoming Checkout
                </div>
                <div class="checkout__list">
                    <table>
                        <?php
                            // get data which customer is closer to check out
                            $custSQL = "SELECT * FROM CUSTOMER ORDER BY CUSTDATE + CUSTPERIOD - CURDATE() ASC";
                            $custResult = mysqli_query($connect, $custSQL);
                            $counter = 1;
                            
                            while ($row = mysqli_fetch_array($custResult)) {
                                // get the date customer need to check out
                                $checkOutDate = date('Y-m-d', strtotime($row["CUSTDATE"] . ' + ' . $row["CUSTPERIOD"] . ' days'));

                                // calculate the number of days left for check out
                                $currentDate = date('Y-m-d');
                                $daysLeft = floor((strtotime($checkOutDate) - strtotime($currentDate)) / (60 * 60 * 24));

                                // if the days left less than 3 days make the row red
                                if ($daysLeft <= 3) {
                        ...
                        ?>
                            ...
                            <td><?php echo $row["CUSTROOMNUM"] ?></td>
                            <td><?php echo $daysLeft ?> day(s) left</td>
                        </tr>
                        <?php
                            }
                        ?>
                ...
                <div class="room__chart">
                    <div class="test">
                    <div id="donutchart"></div>
                    </div>
                </div>
                <div class="sale__chart">
