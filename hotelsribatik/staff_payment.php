<?php
    ...
    $period = $_SESSION['custPeriod'];

    //error controller if enter payment page without roomnumber session
    if($roomNumber == null){
        echo "<script>alert('Please check out room first before make payment!'); window.location.href = 'staff_checkout.php';</script>";
    }

    //find the room price
    $priceSQL = "SELECT ROOMPRICE FROM ROOM WHERE ROOMNUMBER = '$roomNumber'";
    $priceResult = mysqli_query($connect, $priceSQL);
    ...
    //calculate the room price and period of stay
    $totalPayment = $roomPrice * $period;
?>
<html>
    <head>
        <title>Payment | Hotel Sri Batik</title>
    ...
    <nav class="navbar">
        ...
        <main>
            ...
            <div class="wrapper">
                ...
                <form class="form" action="payment_function.php" method="POST">
                    <div class="inputfield">
                        <label>Room Number</label>
                        <input type="text" class="input" readonly value="<?php echo $roomNumber ?>" name="roomNum" required>
                    </div>
                    <div class="inputfield">
                        <label>Customer Name</label>
                        <input type="text" class="input" readonly value="<?php echo $custName ?>" name="custName" required> 
                    </div> 
                    <div class="inputfield">
                        <label>Period of Stay (night)</label>
                        <input type="number" class="input" readonly value="<?php echo $period ?>" name="period" required>
                    </div> 
                    <div class="inputfield">
                        <label>Room Price</label>
                        <input type="text" class="input" readonly value="<?php echo "RM " . $roomPrice ?>" name="roomPrice" required>
                    </div> 
                    <div class="inputfield">
                        <label>Total Payment</label>
                        <input type="text" class="input" readonly value="<?php echo "RM " . number_format($totalPayment, 2) ?>" name="totalPayment" required>
                    </div> 
                    <div class="inputfield">
